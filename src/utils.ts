...
} from "./application/types/analysis-results";
import { CodeLocation } from "./domain/code/code-element.js";

export function formatTypeAnalysis(result: TypeRelationshipResult): string {
  return `
...
export function formatTypeUsages(usages: CodeLocation[]): string {
  return `
  ...
  ${usages
    .map(
      (usage, index) =>
        `${index + 1}. Lines ${usage.startLine}-${usage.endLine}`
    )
    .join("\n")}
  `;
}
