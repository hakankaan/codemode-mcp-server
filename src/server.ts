...
import { TypeAnalysisService } from "./application/analysis/type-analysis-service.js";
import { TypeAnalysisFactory } from "./application/analysis/type-analysis-factory.js";

export class CodemodeServer {
  private server: Server;
  private analysisService: AnalysisService;
  private typeAnalysisService: TypeAnalysisService;
      ...
      tools: [
        {
          ...
          description:
            "Get a hierarchical view of files and subfolders in a directory...",
          inputSchema: {
            type: "object",
            properties: {
              folderPath: {
        ...
        },
        {
          ...
          description:
            "Get functions, classes, interfaces, and variables in a file",
          inputSchema: {
            type: "object",
            properties: {
              filePath: {
...

        {
          ...
          description:
            "Get code elements (functions, classes, etc.) from multiple files in a single request. More efficient than making multiple single-file requests.",
          inputSchema: {
            type: "object",
            properties: {
              files: {
              ...
              maxConcurrent: {
                type: "number",
                description:
                  "Maximum number of files to process concurrently. Defaults to 5.",
                minimum: 1,
                maximum: 20,
              },
            },
        ...
        },
        {
          ...
          description:
            "Get content of multiple code elements from multiple files in a single request. Optimizes performance for bulk operations.",
          inputSchema: {
            type: "object",
            properties: {
              elements: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    elementName: {
                      type: "string",
                      description:
                        "Name of the element (function, class, etc.)",
                    },
                  },
                  required: ["filePath", "elementName"],
                },
              ...
              maxConcurrent: {
                type: "number",
                description:
                  "Maximum number of elements to process concurrently. Defaults to 5.",
                minimum: 1,
                maximum: 20,
              },
            },
      ...
      try {
        switch (name) {
          case "get-folder-structure": {
            ...
            const structure =
              await this.analysisService.getProjectStructure(folderPath);
            return {
              content: [
                {
                  type: "text",
          ...
          case "get-code-elements": {
            ...
            const elements =
              await this.analysisService.getCodeElements(filePath);
            return {
              content: [
                {
                  type: "text",
          ...
          case "get-element-content": {
            const filePath = args.filePath as string;
            const elementName = args.elementName as string;

            if (!filePath || !elementName) {
              throw new Error("filePath and elementName are required");
            }

            const content = await this.analysisService.getElementDetails(
              filePath,
              elementName
            );
            ...
            if (!content) {
              throw new Error(
                `Element ${elementName} not found in ${filePath}`
              );
            }
          ...
          case "get-method-details": {
            ...
            if (!filePath || !className || !methodName) {
              throw new Error(
                "filePath, className, and methodName are required"
              );
            }

            const method = await this.analysisService.getMethodDetails(
              filePath,
              className,
              methodName
            );
            ...
            if (!method) {
              throw new Error(
                `Method ${methodName} not found in ${className} in ${filePath}`
              );
            }
          ...
          case "batch-get-code-elements": {
            const files = args.files as Array<{ filePath: string }>;
            const maxConcurrent = (args.maxConcurrent as number) || 5;

            if (!Array.isArray(files) || files.length === 0) {
              throw new Error("files must be a non-empty array");
            }

            const result = await this.analysisService.batchGetCodeElements(
              files,
              maxConcurrent
            );
